...
const slice = createSlice({
    ...
    reducers: {
        removeTaskAC(state, action: PayloadAction<{ taskId: string, todolistId: string }>) {
            const taskIndex = state[action.payload.todolistId].findIndex((taks) => taks.id === action.payload.taskId)
            if (taskIndex > -1) {
        ...
        updateTaskAC(state, action: PayloadAction<{ taskId: string, domainModel: UpdateDomainTaskModelType, todolistId: string }>) {
            const taskIndex = state[action.payload.todolistId].findIndex((taks) => taks.id === action.payload.taskId)
            if (taskIndex > -1) {
                state[action.payload.todolistId][taskIndex] = { ...state[action.payload.todolistId][taskIndex], ...action.payload.domainModel }
            }
        },
